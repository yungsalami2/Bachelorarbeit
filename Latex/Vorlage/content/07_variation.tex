...
Frenkel (1934) hat Diracs Ansatz dementgegen als Minimierungsbedinung (siehe Gl. (\ref{Residuum})) und dadurch Orthogonalprojektion interpretiert (DFVP) und 
...
zu verwenden (siehe oben in der historischen Randnotiz), in der das Residuum der Schrödingergleichung bzw. der Abstand zwischen der Schrödingergleichung und der Tangentialebene $T_{\mu(t)}M$ 
...
mit dem Winkel $\Theta$ in \autoref{fig:qubit}. Der Imaginärteil $Im[\mu]$ beschreibt dabei die Phase der Zustände und steht deshalb ebenfalls in 
Beziehung mit dem Winkel $\Phi$ in \autoref{fig:qubit}.\\
...
Hamiltonian (\ref{zweSpinHamilton}) zu:
\begin{align}
